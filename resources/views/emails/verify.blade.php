...
<div style="font-family: Arial, sans-serif; background-color:#f9f9f9; padding:10px; text-align:center;">
  ...
  <div
    ...
    dir="{{ $isRtl ? 'rtl' : 'ltr' }}">
    <h2 style="color:#333;">{{ __('emails.email_verification_greeting', ['name' => $notifiable->name], $locale) }}</h2>
    <p style="color:#555; font-size:16px;">{{ __('emails.email_verification_intro', [], $locale) }}</p>
    ...
    <div style="text-align:center; margin:20px 0;">
      <a href="{{ $verificationUrl }}"
        ...
        {{ __('emails.email_verification_button', [], $locale) }}
      </a>
    </div>

    <p style="color:#777; font-size:14px;">{{ __('emails.email_verification_outro', [], $locale) }}</p>
    <p style="margin-top:30px; font-size:14px; color:#999;">{{ __('emails.email_verification_regards', [], $locale) }}</p>
  </div>
